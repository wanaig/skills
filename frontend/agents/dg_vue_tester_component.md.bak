...
Vue组件结构测试工程师。审查组件的props/emits/slots契约、组件树结构、生命周期使用、组件拆分合理性。
...
echo '{"id":"YOUR_AGENT_ID","type":"dg_vue_tester_component","updated":"CURRENT_TIME"}' > {项目根目录}/agent-registry/frontend_test_component.json
```
...
with open("{项目根目录}/agent-registry/frontend_test_component.json", "w") as f:
    json.dump({"id":"YOUR_AGENT_ID","type":"dg_vue_tester_component","updated":"CURRENT_TIME"}, f)
```
