...
**B. 架构演进路径**：描述数据架构的演进方向
```markdown
V1（MVP）：{单库 + 基础缓存}
V2（成长期）：{读写分离 + 引入 ES/搜索引擎}
V3（成熟期）：{分库分表 + 多级缓存 + 数据湖}
```
...
| 故障场景 | 影响 | 降级策略 |
|---------|------|---------|
| 数据库主库宕机 | 写入不可用 | 主从切换 + 读降级到从库 |
| 缓存雪崩 | 数据库压力暴增 | 缓存预热 + 互斥锁 + 随机 TTL |
| 数据误删 | 数据丢失 | 备份恢复 + 软删除 + 延迟删除 |
```

**E. 容量规划**：
```markdown
| 实体 | 预估行数（1年） | 存储量 | 索引量 | 分表策略 |
|------|---------------|--------|--------|---------|
| {核心实体1} | {M} 万 | {N} GB | {X} GB | {按时间/userId 等} |
| {核心实体2} | {M} 万 | {N} GB | {X} GB | |
```
...
| 迁移场景 | 触发条件 | 工具 | 步骤概要 |
|---------|---------|------|---------|
| 数据库类型迁移 (PG→MySQL) | 云厂商限制 | {工具} | {步骤} |
| 分表迁移 | 单表超 {N} 万行 | pt-online-schema-change / gh-ost | {步骤} |
```
