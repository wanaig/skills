...
- 文件路径：`{PROJECT_ROOT}/outputs/agent-registry/fa_api_design.json`
- 内容格式：
...
- [ ] 已将 Agent ID 写入 `agent-registry/fa_api_design.json`

### 6. 深度评审能力（Phase 3 调用时使用）

当主Agent 在 Phase 3 深度评审阶段调用你时，需在 v2 基础上补充以下内容：

**A. ADR 记录**：为每个关键 API 设计决策按以下格式记录：
```markdown
## ADR-API-{编号}：{决策标题}
- 背景：这个 API 设计决策的背景是什么？
- 选项：(a) {方案A} (b) {方案B}
- 决策：选择 {X}
- 理由：{前端消费体验/性能/可维护性/标准兼容}
- 后果：{正面 + 负面 + 对前后端开发的影响}
```

**B. 架构演进路径**：API 设计演进方向
```markdown
V1（MVP）：RESTful CRUD + 基础认证 + 偏移分页
V2（成长期）：批量操作 + 高级筛选 + 字段裁剪（sparse fieldsets）+ 部分 WebSocket 端点
V3（成熟期）：API 版本化升级 + BFF 层 + 自动生成 SDK + GraphQL 探索（按需）
```
...
| API 故障场景 | 影响 | 降级策略 |
|-------------|------|---------|
| 某端点响应超时 | 前端页面白屏 | 设置合理超时 + 前端展示降级内容 + 熔断器 |
| 认证服务不可用 | 所有需认证的请求失败 | 延长已登录用户的 Token 有效期 + 降级提示 |
| 依赖的第三方 API 不可用 | 关联功能失效 | 缓存上次成功响应 + 异步重试 |
```

**E. 容量规划**：API 容量估算
```markdown
| 端点类型 | 预估 QPS | P95 延迟目标 | 限流阈值 | 缓存策略 |
|---------|---------|------------|---------|---------|
| 认证端点 | {N} /s | < 200ms | 5次/分钟/IP | 不缓存 |
| 查询列表 | {N} /s | < 300ms | 100次/分钟/用户 | CDN + Redis (30s) |
| 写入端点 | {N} /s | < 500ms | 50次/分钟/用户 | 不缓存 |
```
...
| 迁移场景 | 触发条件 | 步骤概要 |
|---------|---------|---------|
| REST v1 → v2（不兼容变更） | 字段重命名/删除/类型变更 | 新增 /api/v2 端点 + v1 标为 deprecated + 设置废弃期限 + 监控 v1 调用量归零后下线 |
| 偏移分页 → 游标分页 | 数据量大到偏移分页性能瓶颈 | 新增游标分页参数 + 两套参数并行 + 前端逐步切换 |
| REST → 部分 GraphQL | 前端需要灵活字段组合 | 新增 GraphQL 网关层 + 底层复用 REST 逻辑 + 渐进替换 |
```
