...
- 文件路径：`{PROJECT_ROOT}/outputs/agent-registry/fa_security.json`
- 内容格式：
...
- 决策：选择 {X}
- 理由：{安全性/性能/用户体验/合规}
- 后果：{正面保护效果 + 负面性能/UX影响 + 运维成本}
```

**B. 架构演进路径**：安全架构演进方向
```markdown
V1（MVP）：JWT 认证 + RBAC 鉴权 + TLS 传输加密 + 基础限流
V2（成长期）：MFA 多因素 + 审计日志完善 + WAF + 依赖安全扫描
V3（成熟期）：零信任架构 + SIEM + 自动化渗透测试 + SOC2/等保认证
```
...
| 安全故障场景 | 影响 | 降级策略 |
|-------------|------|---------|
| JWT 签名密钥泄露 | 所有 Token 可伪造 | 密钥轮换 + 全部 Token 吊销 + 用户重新登录 |
| Redis 黑名单不可用 | Token 吊销失效 | 降级为仅依赖 Token 过期时间 + 缩短 Access Token 时效 |
| 第三方 OAuth 不可用 | 第三方登录失败 | 回退到密码登录 + 提示用户 |
```
...
| 安全组件 | 预估负载 | 规格需求 |
|---------|---------|---------|
| Token 签发 | {N} 次/秒峰值 | JWT 签名性能 > {N} ops/s |
| Token 黑名单（Redis） | {M} 个活跃 Token | 每个 Token 记录 ~200 bytes |
| 审计日志 | {N} 条/天 | 存储 {X} GB/月，保留期 {Y} 天 |
| 限流计数器（Redis） | {N} 个 key/秒 | 内存估算 {X} MB |
```
...
| 迁移场景 | 触发条件 | 步骤概要 |
|---------|---------|---------|
| Session → JWT | 需要无状态/跨服务认证 | 双写过渡期 + 逐步迁移验证 + 清理旧 Session |
| 自建认证 → OAuth2/OIDC | 需要 SSO/企业集成 | 接入新认证 + 用户关联 + 废弃旧密码 |
| bcrypt → argon2 | 安全要求提升 | 登录时自动重新哈希 + 标记旧哈希用户 |
```
