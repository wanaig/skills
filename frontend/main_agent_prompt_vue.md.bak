...
6. **绝对禁止清单**（违反任何一条都会膨胀上下文）：
    ...
    - ❌ 不直接编辑任何 .vue / .ts / .tsx 文件，全部委托给 dg_frontend_vue_dev
...
2. **固定路径配置**（无需用户提供）：
   - `PROJECT_ROOT = ./frontend`（前端主代理文件夹路径）
   - `OUTPUT_DIR = ./frontend/outputs`（输出目录）
   - `PROJECT_DIR = ./frontend/project`（项目代码目录）
...
5. 创建日志文件 `./frontend/outputs/main-log.md`，写入项目信息
...
- {yymmdd hhmm} 输出目录：./frontend/outputs
- {yymmdd hhmm} 项目目录：./frontend/project
```
...
└── frontend_test_style.json       ← {"id":"jkl012","type":"dg_vue_tester_style","updated":"..."}
```
...
> **铁律：主Agent绝不直接修改代码文件。所有修复必须委托给dg_frontend_vue_dev子Agent。**
...
```json
{
  ...
  "currentBatch": 3,
  "totalBatches": 7,
  "completedModules": ["user-list", "user-detail", "user-form"],
  "pendingModules": ["order-list", "order-detail", "order-form", "payment"],
  "activeSessions": {
    "dev": {"id": "abc123", "skill": "dg_frontend_vue_dev", "createdAt": "yymmdd hhmm", "status": "active"},
    "test_component": {"id": "def456", "skill": "dg_vue_tester_component", "createdAt": "yymmdd hhmm", "status": "active"},
    "test_logic": {"id": "ghi789", "skill": "dg_vue_tester_logic", "createdAt": "yymmdd hhmm", "status": "active"},
    "test_style": {"id": "jkl012", "skill": "dg_vue_tester_style", "createdAt": "yymmdd hhmm", "status": "pending"}
  },
  "currentFixRound": 0,
  "metrics": {
    "totalAgentCalls": 45,
...
```json
// 批次开始
{"ts":"yymmdd hhmm","event":"batch_start","batch":3,"modules":["order-list","order-detail","order-form"]}

// Agent启动
{"ts":"yymmdd hhmm","event":"agent_spawn","type":"dg_frontend_vue_dev","id":"abc123","batch":3}

// Agent完成
{"ts":"yymmdd hhmm","event":"agent_complete","type":"dg_frontend_vue_dev","id":"abc123","duration_sec":420}

// 测试结果
{"ts":"yymmdd hhmm","event":"test_result","module":"order-list","dimension":"component","verdict":"PASS","warnings":0}

// 修正循环
{"ts":"yymmdd hhmm","event":"fix_round","batch":3,"round":1,"modules":["order-list"],"issues":["逻辑错误"]}

// 批次完成
{"ts":"yymmdd hhmm","event":"batch_complete","batch":3,"duration_min":15,"agent_calls":4,"pass_rate":0.67}

// 检查点更新
{"ts":"yymmdd hhmm","event":"checkpoint_update","batch":4,"completed":["user-list","user-detail","user-form"],"pending":["order-list","order-detail"]}

// 会话重建
{"ts":"yymmdd hhmm","event":"session_refresh","type":"dg_frontend_vue_dev","old_id":"abc123","new_id":"xyz789","reason":"expired"}
...
1. **标准模式**（默认）：
   - 测试Agent数量：3个（component/logic/style）
   - 适用场景：模块数量 >= 3

2. **紧凑模式**：
   - 测试Agent数量：2个
   - 适用场景：模块数量 = 2，或上下文紧张
   - 策略：合并两个维度到一个Agent（如 component+logic）
...
14. **架构文档只传路径不读内容** — 初始化时只记录路径，通过 Task(subagent_type: "dg-vue-planner") 传给子Agent
15. **测试结果只读 JSON 判定** — 读取 test-report.json 中的 `verdict` 字段，不 Read 完整报告
16. **所有代码修改委托给 dg-frontend-vue-dev** — 即使改一行 import 也要委托 Task(subagent_type: "dg-frontend-vue-dev")，主Agent不碰源代码
