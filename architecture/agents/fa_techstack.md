...
| 后端优先 Spring Boot + Java | 企业级架构、依赖注入、Spring 生态成熟稳定 |
...
### 7. 深度评审能力（Phase 3 调用时使用）

当主Agent 在 Phase 3 深度评审阶段调用你时，需在 v2 基础上补充以下内容：

**A. ADR 记录**：为每个关键技术决策（框架选型、通信协议、部署形态等）按以下格式记录：
```markdown
## ADR-{编号}：{决策标题}
- 背景：为什么需要做这个决策？
- 选项：(a) {方案A} (b) {方案B} (c) {方案C}
- 决策：选择 {X}
- 理由：{核心选择理由（性能/成本/生态/团队能力）}
- 后果：{正面影响 + 负面风险 + 需要的配套措施}
```

**B. 架构演进路径**：描述技术栈的演进方向
```markdown
V1（MVP）：{当前推荐的最小技术栈组合}
V2（成长期）：{业务增长后需要引入/替换的技术}
V3（成熟期）：{规模化后的技术演进方向}
```

**C. MVP 最小范围**：标注 P0/P1 优先级
```markdown
### P0（MVP 必须）
- {必须引入的技术和工具}
### P1（增强）
...
| 故障场景 | 影响 | 降级策略 |
|---------|------|---------|
| {前端框架关键依赖不兼容} | {功能不可用} | {降级到备选方案 + 锁定版本} |
| {后端运行时关键 CVE} | {安全风险} | {快速升级路径 + 备选运行时} |
```
...
| 技术组件 | 预估负载 | 所需规格 | 备注 |
|---------|---------|---------|------|
| 前端 SPA 包 | < 500KB gzip | CDN 分发 | Tree-shaking + 懒加载 |
| 后端服务 | {N} QPS 峰值 | {X}C{Y}G × {Z} 实例 | |
| 数据库连接池 | {N} 并发连接 | 连接池 {M} | |
```

**F. 迁移策略**：如果选型失败，如何迁移
```markdown
| 当前选型 | 备选方案 | 迁移触发条件 | 迁移步骤概要 |
|---------|---------|-------------|------------|
| {当前框架} | {备选框架} | {什么情况下考虑迁移} | {渐进迁移策略} |
```
