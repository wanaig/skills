...
启动 general 作为智能合约开发工程师：

```
Task(subagent_type: "general", run_in_background: true, prompt: "请先阅读文件：{file:./blockchain/agents/bc_solidity_dev.md}，然后按照文件中的指导执行以下任务。\n\n开发任务：{合约列表}\ndev-plan: {路径}\ncontract-design-guide: {路径}\ntech-stack: {路径}\nlessons-learned: {路径}\n项目根目录: {PROJECT_ROOT}/project\n需求文档路径：{REQUIREMENT_FILE}\n\n请按顺序逐个合约开发。")
```
...
```
Task(subagent_type: "general", run_in_background: true, prompt: "请先阅读文件：{file:./blockchain/agents/bc_tester_functional.md}，然后按照文件中的指导执行以下任务。\n\n功能测试：{合约列表}\n待测项目：{PROJECT_ROOT}/project\ncontract-design-guide: {路径}\n输出目录: {PROJECT_ROOT}/outputs/bc_tester_functional/\n\n测试报告同时输出 markdown 和 JSON 格式。")

Task(subagent_type: "general", run_in_background: true, prompt: "请先阅读文件：{file:./blockchain/agents/bc_tester_security.md}，然后按照文件中的指导执行以下任务。\n\n安全测试：{合约列表}\n待测项目：{PROJECT_ROOT}/project\ncontract-design-guide: {路径}\n输出目录: {PROJECT_ROOT}/outputs/bc_tester_security/\n\n测试报告同时输出 markdown 和 JSON 格式。")

Task(subagent_type: "general", run_in_background: true, prompt: "请先阅读文件：{file:./blockchain/agents/bc_tester_gas.md}，然后按照文件中的指导执行以下任务。\n\n燃耗测试：{合约列表}\n待测项目：{PROJECT_ROOT}/project\ncontract-design-guide: {路径}\n输出目录: {PROJECT_ROOT}/outputs/bc_tester_gas/\n\n测试报告同时输出 markdown 和 JSON 格式。")
```
