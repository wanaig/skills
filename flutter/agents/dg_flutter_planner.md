...
0. **项目现有结构**（增量开发场景）：如果项目目录已存在代码：
   - 用 Glob 扫描 `{PROJECT_ROOT}/project/lib/` 下的完整目录结构
...
**创建目录**：
```bash
mkdir -p {PROJECT_ROOT}/project/lib/{config,models,providers,repositories,services,screens,widgets/ui,utils}
```
...
```
计划完成，产出文件：
- {PROJECT_ROOT}/project/pubspec.yaml
- {PROJECT_ROOT}/outputs/dg_flutter_planner/dev-plan.md
- {PROJECT_ROOT}/outputs/dg_flutter_planner/design-guide.md
- {PROJECT_ROOT}/outputs/dg_flutter_dev/lessons-learned.md
- {PROJECT_ROOT}/project/lib/app.dart
- {PROJECT_ROOT}/project/lib/main.dart
- {PROJECT_ROOT}/project/lib/models/api_response.dart
- {PROJECT_ROOT}/project/lib/services/api_client.dart
- {PROJECT_ROOT}/project/lib/services/secure_storage.dart
- {PROJECT_ROOT}/project/lib/config/app_config.dart
- {PROJECT_ROOT}/project/lib/config/theme.dart
- {PROJECT_ROOT}/outputs/（含各 Agent 产出目录）
- {PROJECT_ROOT}/project/lib/ (项目目录结构已就绪)
